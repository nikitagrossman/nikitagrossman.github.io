...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const updateChart = async () => {
        const getData = await fetch(`https://min-api.cryptocompare.com/data/pricemulti?fsyms=${coinsToShow.join(',')}&tsyms=USD`);
    ...
    $(document).on('change', '.track-switch', async function () {
        ...
        let trackedCoinsFromStorage = JSON.parse(localStorage.getItem('trackedCoins') ?? '[]');
        if ($(this).is(':checked')) {
            const coinResponse = await fetch(`https://min-api.cryptocompare.com/data/pricemulti?fsyms=${coinSymbolWasChosen}&tsyms=USD`);
            const coinData = await coinResponse.json();
            if (trackedCoinsFromStorage.length < 5 &&
                ...
                localStorage.setItem('trackedCoins', JSON.stringify(trackedCoinsFromStorage));
            }
            else {
                if (coinData.Response === 'Error') {
                    alert('Coin cannot be tracked at the moment.');
                ...
                trackedCoinsFromStorage.forEach((coinId) => {
                    $('.modal-body').append(`<p>${coinId}</p> <button class="btn btn-danger remove-coin" data-coin-id="${coinId}">Remove</button>`);
                });
